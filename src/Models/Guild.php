 ...
 *
 * @property string|null                                                     $rulesChannelID               The ID of the rules channel, or null.
 * @property \CharlotteDunois\Yasmin\Interfaces\GuildChannelInterface|null   $rulesChannel                 The guild's rules channel, or null.
 * @property int|null                                                        $premiumTier                  The premium tier level of this guild.
 * @property int|null                                                        $premiumCount                 The number of boosts this guild has.
 * @property string|null                                                     $preferredLocale              The preferred locale of this server
 * @property string|null                                                     $publicUpdatesChannelID       Public updates channel ID.
 * @property \CharlotteDunois\Yasmin\Interfaces\GuildChannelInterface|null   $publicUpdatesChannel         Public updates channel.
 */
class Guild extends ClientBase {
    ...
    /**
     * The ID of the rules channel, or null.
     * @var string|null
     */
    protected $rulesChannelID;

    /**
     * Public updates channel ID, or null.
     * @var string|null
     */
    ...
    /**
     * The preferred locale of this server
     * @var string|null
     */
    ...
    function __get($name) {
        ...
        switch ($name) {
            ...
            break;
            case 'rulesChannel':
                return $this->channels->get($this->rulesChannelID);
            break;
            case 'publicUpdatesChannel':
                return $this->channels->get($this->publicUpdatesChannelID);
            break;
            case 'vanityURL':
    ...
    function edit(array $options, string $reason = '') {
        return (new \React\Promise\Promise(function(callable $resolve, callable $reject) use ($options, $reason) {
            $data = \CharlotteDunois\Yasmin\Utils\DataHelpers::applyOptions($options, array(
                ...
                'rulesChannel' => array('key' => 'rules_channel_id', 'parse' => function($val) {
                    return ($val instanceof \CharlotteDunois\Yasmin\Models\TextChannel ? $val->id : $val);
                }),
                'publicUpdatesChannel' => array('key' => 'public_updates_channel_id', 'parse' => function($val) {
                    return ($val instanceof \CharlotteDunois\Yasmin\Models\TextChannel ? $val->id : $val);
                }),
                'preferredLocale' => array('key' => 'preferred_locale', 'type' => 'string'),
    ...
    /**
     * Edits the rules channel of the guild. Resolves with $this.
     * @param string|\CharlotteDunois\Yasmin\Models\TextChannel|null  $channel
     * @param string                                                  $reason
     * @return \React\Promise\ExtendedPromiseInterface
     */
    function setRulesChannel($channel, string $reason = '') {
        return $this->edit(array('rulesChannel' => $channel), $reason);
    }


    /**
     * Edits the public updates channel of the guild. Resolves with $this.
     * @param string|\CharlotteDunois\Yasmin\Models\TextChannel|null  $channel
     * @param string                                                  $reason
     * @return \React\Promise\ExtendedPromiseInterface
     */
    function setPublicUpdatesChannel($channel, string $reason = '') {
        return $this->edit(array('publicUpdatesChannel' => $channel), $reason);
    }
    ...
    function _patch(array $guild) {
        ...
        $this->rulesChannelID = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($guild['rules_channel_id'] ?? $this->rulesChannelID), 'string');
        $this->publicUpdatesChannelID = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($guild['public_updates_channel_id'] ?? $this->publicUpdatesChannelID), 'string');
        $this->preferredLocale = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($guild['preferred_locale'] ?? $this->preferredLocale), 'string');
        $this->premiumTier = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($guild['premium_tier'] ?? $this->premiumTier), 'int');
        $this->premiumCount = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($guild['premium_subscription_count'] ?? $this->premiumCount), 'int');
